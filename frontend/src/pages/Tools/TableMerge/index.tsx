...
export default function TableMergePage() {
  ...
  const waitForArchive = async (jobId: string, partSizeMb: number) => {
    for (;;) {
      const response = await api.get<ArchiveStatus>('/api/tools/vulnerability-process/' + jobId + '/archive/status', { params: { part_size_mb: partSizeMb, archive_format: 'zip_lzma' } })
      const result = response.data
      if (result.status === 'completed') return result.parts || []
      if (result.status === 'failed') throw new Error(result.error || '压缩分卷失败')
  ...
  const handleArchive = async (jobId: string) => {
    ...
    try {
      ...
      const response = await api.post<ArchiveStatus>('/api/tools/vulnerability-process/' + jobId + '/archive', form)
      const parts = response.data.status === 'completed' ? (response.data.parts || []) : await waitForArchive(jobId, archiveSizeMb)
      setArchiveParts(parts)
      Toast.success(`已生成 ${parts.length} 个压缩分卷`)
    } catch (error: any) {
  ...
  const sendEmail = async () => {
    ...
    try {
      ...
      archiveForm.append('part_size_mb', String(emailPartSizeMb))
      archiveForm.append('archive_format', 'zip_lzma')
      const archiveResponse = await api.post<ArchiveStatus>('/api/tools/vulnerability-process/' + emailJobId + '/archive', archiveForm)
      if (archiveResponse.data.status !== 'completed') await waitForArchive(emailJobId, emailPartSizeMb)
    ...
    <div className="table-merge-page">
      ...
      >
        <div className="archive-settings">
          ...
          <Button type="primary" loading={archiveLoading} onClick={() => handleArchive(archiveJobId)}>生成分卷</Button>
        </div>
        <p className="archive-hint">首次压缩在后台执行，完成后结果会缓存；相同报告和分卷大小不会重复压缩。默认按 20MB 分卷。</p>
